...
class WooInstance(models.Model):
    ...
    def login(self):
        try:
            ...
            version = 'wc/v3'
            wcapi = API(url=location, consumer_key=cons_key, consumer_secret=sec_key, wp_api=True, version=version,timeout=10)
            r = wcapi.get("")
        except Exception as e:
            raise UserError(
                _("Connection Unsuccessful..!! \nPlease check your Url,Consumer Key or Consumer Secret / Refresh Connection.. got : " + str(e)))

        if r.status_code == 200:
            raise UserError(_("Congratulations, WooCommerce and Odoo connection has been successfully established"))
        else:
            raise UserError(
                _(f"Connection Unsuccessful..!! \nPlease check your Url,Consumer Key or Consumer Secret / Refresh Connection..got {r.status_code} with {r.__dict__}"))
